  ...
  useEffect(() => {
    ...
    const loadRecord = async () => {
      ...
      if (taskError || !taskData) {
        ...
        setErrorMessage("Could not load task information.");
        setLoading(false);
        return;
      }
      ...
      const { data: laneData, error: laneError } = await supabase
        .from("ew_lanes")
        .select("id, task_id, lane_key, title, sort_order")
      ...
      const laneIds = (laneData as Lane[]).map((lane) => lane.id);
                      ...
                      return (
                        <div key={post.id} className="px-5 py-4">
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            <button
                              type="button"
                              onClick={() => handleToggleLearning(post, lane.title)}
                            ...
                            <div className="mt-4 space-y-3">
                              ...
                              {learning &&
                                ...
                                suggestionStyles.map((style) => (
                                  <div
                                    key={style.key}
                                    className={`overflow-hidden rounded-xl border ${style.border}`}
                                  >
